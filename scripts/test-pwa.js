...
// docs/features/pwa.md: the manifest is reachable at /manifest.webmanifest
// with the right MIME, the SW is reachable at /sw.js with the
// Service-Worker-Allowed + no-cache headers, the index.html links to
// the manifest + apple-touch-icon, and the icons resolve. The app is
...
server.listen(0, '127.0.0.1', () => {
  ...
  (async () => {
    try {
      // 1. GET / -> index.html with manifest + apple-touch-icon <link>s.
      const home = await req('GET', '/');
      assert(home.status === 200, 'home: status 200');
      assert(/<link rel="manifest"/i.test(home.body), 'home: <link rel="manifest"> present');
      assert(/href="\/manifest\.webmanifest"/i.test(home.body), 'home: manifest href');
      ...
      // 2. GET /manifest.webmanifest -> JSON-ish, correct MIME, must declare icons.
      ...
      assert(m.id === '/', 'manifest: stable same-origin app id');
      assert(m.start_url === '/', 'manifest: start_url');
      assert(m.scope === '/', 'manifest: scope');
      ...
      for (const size of ['192x192', '512x512']) {
        ...
        assert(/^\/icons\//.test(hit.src), 'manifest: icon src is absolute at the root (/icons/...)');
      }

      // 3. GET /sw.js -> JS body + Service-Worker-Allowed + no-cache.
      ...
      assert(sw.headers['service-worker-allowed'] === '/', 'sw: Service-Worker-Allowed: /');
      ...
      for (const rel of ['/icons/icon-192.png', '/icons/icon-512.png', '/icons/icon-maskable-512.png']) {
