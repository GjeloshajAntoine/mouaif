...
- [CLI commands](features/cli-commands.md) — every `mouaif` command and `serve` option, and why no install needs `npm run build:web`.
...
- [Image generation](features/image-generation.md) — the agent draws a picture with an image model, saves it as a project file, and receives the image itself; a subagent can generate and the main agent gets the file plus the picture. Ships as a leaf of the File tools group, gated by the File tools authorization.
