...
- [Inspector](features/inspector.md) — from-scratch mobile-friendly DevTools-style UI on top of CDP over WebSocket. Console + Network panels driven by a virtual list; targets list with a type chip + overflow menu + auto-discovery on mount; tab management (reload / navigate / close). Mobile shell's fourth tab.
