...
- The Actions category is populated only by `GET /api/actions`; native model tools such as `read_file`, `ask_user`, and `task` are excluded because they are not saved project actions and are not directly invocable through `@`. Action rows show their user-facing label and description rather than the underlying CLI/MCP implementation. Tool catalog entries with `kind: "mcp"` use the MCP category and include their server slug in the subtitle.
