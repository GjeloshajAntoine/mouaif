...
- Tool catalog entries are split by `kind`: native tools use the Actions category, while `kind: "mcp"` tools use the MCP category and include their server slug in the subtitle. Project custom actions remain Actions even when their saved implementation invokes MCP.
