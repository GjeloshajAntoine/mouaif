...
Shell and subagent call cards auto-expand while running (live output/nested activity is only visible when expanded since the CSS exception for always-visible call bodies was removed) and fold back on a successful result unless the user manually collapsed them; errors auto-expand as before. A settled subagent card is also left expanded (unless the user collapsed it), because its body IS the nested chat — but that is a renderer decision, not a CSS override: `is-expanded` remains the single source of truth for body visibility. An earlier revision excluded subagent cards from the collapsed-body `display: none` rule *and* forced `display: block` on `.tool-card--subagent .tool-card__body`, so collapsing one changed nothing on screen; both were removed in **fix: honor `is-expanded` on subagent tool cards**. `scripts/test-subagent-expand-state.js` locks the cascade.
