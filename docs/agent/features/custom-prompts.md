...
| `POST` | `/api/prompts` | Create a prompt. Body: `{ projectDir?, scope?, title?, content, preset? }` |
| `PATCH` | `/api/prompts/:id` | Update a prompt. Body: `{ projectDir?, scope?, title?, content?, preset? }` |
