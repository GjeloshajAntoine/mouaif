...
The footer's auto-retry control is a `.tool-popup__foot-row` `<label>` (flex row, `min-height: 44px`) wrapping a `span.switch.switch--sm` and the `.tool-popup__foot-label` text as siblings. The label text must stay *outside* the `.switch` box: `.switch--sm` is a fixed 34 × 20 px box, and text nested inside it wrapped one word per line under the track and spilled over the footer note.
