...
> Agent-facing reference for [`docs/features/authentication.md`](../../features/authentication.md), which covers **app access only**. Provider credentials moved to [`docs/features/providers.md`](../../features/providers.md) → [`providers.md`](./providers.md). The published build redirects the old URLs (`REDIRECTED_SLUGS` in `scripts/build-docs.js`): `auth` → `providers`, `access-authentication` → `authentication`.
