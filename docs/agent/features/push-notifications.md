...
| `/api/push/subscribe` | POST | Save a push subscription (body: `{ subscription: { endpoint, keys: { p256dh, auth }, statusBarMaxChars? } }`) — `statusBarMaxChars` is the device's measured notification body line, used to size the ASCII status bar |
...
### Device-sized status bar

A status body's first line is the ASCII bar, so a body the OS collapses to a single line still shows progress. `sendChatPush()` passes `bodyFor(sub)` instead of a fixed `body`; `push.sendPush()` resolves that function **per target subscription**, which is what lets one chat's status render at three widths across three devices. The cell count comes from `push.statusBarSizeForMaxChars()` against the `push_subscriptions.status_bar` value the device reported; `push.asciiStatusBar(percent, size)` is the single formatter (`narrow` 6 / `wide` 10 / `huge` 20 cells, an unmeasured device defaulting to `narrow`). The page measures its own body line in `frontend/src/components/push.js` `statusBarMaxChars()` — a hidden monospace probe laid out at the viewport width, so a desktop window reports a larger budget with no user-agent sniffing — and sends it with `POST /api/push/subscribe` as `subscription.statusBarMaxChars`. An omitted value keeps the device's previously stored width rather than resetting it.
