...
- **Self-report.** `frontend/src/components/push.js` `deviceReport()` returns `{ chars, viewportWidth, os, osVersion, style }` and sends it as `subscription.statusBarProfile` (plus the older bare `statusBarMaxChars`) on `POST /api/push/subscribe`. `chars` is *measured* — a hidden monospace probe at the viewport width, so it tracks every phone size without UA sniffing for width. `os`/`osVersion` come from `navigator.userAgentData` where present and fall back to `osFromUserAgent()` (note `MacIntel` + `maxTouchPoints > 1` is reported as `ipados`); `style` comes from `notificationStyle()` (`(display-mode: fullscreen)` → `expanded`).
...
- **Plan.** `statusBarPlan(report)` returns `{ chars, lines, style, layout, detailLines, cells }`. `cells` is continuous in `chars` (clamped to `MIN_CELLS`..`MAX_CELLS`, a stacked bar taking `STACKED_BAR_SHARE` of the line, an inline bar `INLINE_BAR_SHARE`); `lines` comes from `capacityFor(os, osVersion)`, which applies the ordered `versionRules` of the `OS_CAPACITY` table; `layout` is `inline` when `lines === 1`, else `stacked`; `detailLines` is `DETAIL_LINES_INLINE` / `_PREVIEW` / `_EXPANDED` — the height budget that decides how many facts show. A measurement outside `MEASURE_LOWER_BOUND`..`MEASURE_UPPER_BOUND` of what the platform expects for the viewport is clamped and flagged.
- **Content.** `detailLines(plan, percent, info)` builds the rows from the fact object (`{ title, message, kind, current, total, time, tool, model, usage }`) in `DETAIL_TIERS` order, dropping a slot that would wrap. `usageSummary()`, `activityLine()`, `formatTokenCount()`, `formatCost()` (matching `src/usage.js` digits) and `clipSmart()` (tail-preserving clip) are the helpers. `composeStatusBody(plan, percent, info)` renders `stacked` (bar row, then the detail rows) or `inline` (bar + `INLINE_SEPARATOR` + the first detail row, or the bar alone when nothing useful fits). `info` may be a bare string for the one-fact case. `planForSubscription(sub)` reads `status_bar_profile` and falls back to `status_bar`.
- **One unbreakable bar row.** `asciiStatusBar(percent, cells)` joins the cells to the percentage with `PERCENT_GAP` — a non-breaking space (U+00A0), never a plain one. The measured `chars` comes from a 12 px monospace probe, but the OS renders the body in its own larger, proportional font, so the row can exceed the measured line and wrap; with a plain space the break would land between `]` and `40%`, leaving an unlabelled bar and a stray number. The NBSP keeps the pair together, and is one character either way, so `BAR_OVERHEAD_CHARS` and every width budget are unchanged. `scripts/test-status-bar-sizing.js` asserts the gap is the exported constant, that a labelled bar contains no breakable space anywhere, and that the bar row still fits the body line.
- **Facts.** `statusInfo(data, extra)` in `server-handlers-chats.js` merges the stream event with this turn's context (`turnStartedAt`, `lastToolName`, `pushUsageLabel()`), so the handler passes one object and the title stays the chat name. `pushUsageLabel()` writes the usage row, not the title.
...
The former title-only snippet `title: usageLabel ? chatTitle + ' · ' + usageLabel : chatTitle` must not come back: the OS renders the title in a fixed slot and truncates it before the body, so any fact parked there is invisible on the phone it matters most on.
