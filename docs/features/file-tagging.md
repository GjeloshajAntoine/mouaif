...
- **Excerpt format.** `{ start, end }` are 1-indexed inclusive line numbers. An absent excerpt means the whole file. A 0-length excerpt (`start === end`) renders a `# Excerpt: <n>` header and an empty body — useful for "mention this file by location, don't dump it."
