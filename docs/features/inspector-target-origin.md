...
4. **Origin sentence** — one line explaining where the focused value comes from and what editing will do, e.g.   - `padding: 16px is set on element.style, which wins this value for this element only.`
...
- **The bar survives a mode switch.** The Inspector keeps the last non-empty selection the panel published, so switching the Styles panel off — or moving to Console / Network / Info — leaves the element, its rule chips, the origin sentence and the session receipt on screen, and they still describe the element being inspected. Undo from the bar works with the panel off, using the retained `objectId`, and re-reads the element afterwards so the origin sentence reflects the value the undo restored (the only other reader of the page is the panel, which is precisely the one that is not mounted).
...
- **The selection stays owned by the Styles panel.** The bar reads a snapshot the panel publishes (`onSelectionChange`) after each selection or cascade read, and it asks the panel to act through one handle ref (`panelHandlesRef`: `selectAncestor`, `clear`, `refresh`). The panel clears its published snapshot on unmount, and the Inspector retains the last non-empty one — so the bar keeps the element, its rules, the origin sentence **and the session receipt** on screen after the Styles panel is switched off. The panel re-adopts the retained element by `objectId` when it comes back (`restoreObjectId`), which is why a mode switch or an undo performed from the bar leaves the panel showing the same element rather than coming up empty.
