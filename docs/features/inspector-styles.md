...
- The panel is **mobile-first**: every interactive row and control is at least a 44 × 44 px tap target (except the full-width element identity chip, which is a compact 32 px row), read-only rows are shorter, and nothing in the panel scrolls sideways. When Styles is the only visible panel it takes a taller body than the shared default.
