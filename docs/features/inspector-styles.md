...
- The header icon buttons (clear ✕, refresh, pick/stop) are **labelled**, not glyph-only — each shows its word under the icon (`.icon-btn--labeled`). The header wraps (`flex-wrap`) so a label never clips on narrow phones; the buttons keep `aria-label` + `title`. The pick button reads "Pick" off and "Stop" on.
