...
- an **authorization segment** (`Off / Ask / Allow`) — every group carries one, MCP included: the **MCP default** row edits the shared gate (`mcp.mode`), and each MCP server row edits that server's override (`mcp.servers.<slug>`) while showing the *effective* mode. The compact tree does not show a separate override-reset button.
