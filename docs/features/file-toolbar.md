...
The toolbar is a stateless Preact component that receives `projectDir` and `onOpenFileEditor` as props. The git and CLI modals each own their fetch/SSE state (loading / error / retry) and collapse/expand independently.
