...
The trigger icon is a stacked pair of inline SVG triangles (▲ above ▼), rendered with `fill: currentColor` like every other icon in the app. Each triangle is a separate 12×7 SVG with a 2px gap, so both arrows are clearly visible. Earlier attempts used `▲`/`▼` text glyphs (U+25B2/U+25BC), which render as color emoji on some mobile fonts and collapse into a single visible icon, and a single 14px two-triangle SVG, which was too small to read as two icons.
