...
- **Git** — opens a modal with a header and a body. The header holds a **branch dropdown**, **Push** and **Pull** buttons, plus refresh and close. The body shows four stacked collapsible sections — **Staged changes**, **Unstaged changes**, **Recent commits**, and **Stash**. Every file row, commit, and stash is collapsible; each changed file expands into its diff.
