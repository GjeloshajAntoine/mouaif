...
A single trigger button (folder + ▲▼ arrow icon) next to the textarea opens a dropdown menu:
...
The trigger icon is a stack of three inline SVGs: a folder (Files) with both ▲ and ▼ arrows beneath it, all rendered with `fill: currentColor` like every other icon in the app. Each element is a separate SVG (folder 18×14, arrows 12×7) with 2px gaps, so all three are clearly visible. Earlier attempts used `▲`/`▼` text glyphs (U+25B2/U+25BC), which render as color emoji on some mobile fonts and collapse into a single visible icon, and a single 14px two-triangle SVG, which was too small to read as two icons.
