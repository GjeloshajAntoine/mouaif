...
Below the stored-value readout, an **Example** block shows what the model would receive for a representative **file-tool** result under the selected pair. The sample is a realistic monorepo-shaped file list in its compact model-facing form: a small `#` header, then each directory printed once as a `# <dir>/` group header with bare basenames under it, and no raw JSON envelope. The sample is sized (around 70 KiB) so `very-small` and `average` show the head/tail truncation marker the real model path uses, while `full` and `extensive` show the untruncated body.
