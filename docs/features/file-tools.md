...
- Tests: [scripts/test-file-tools.js](../../scripts/test-file-tools.js). 68 assertions covering glob compilation, OpenAI spec shape, `resolveSandbox`, every tool's happy path and every error path (`EOUTSIDE_PROJECT`, `EBADINPUT`, `ETOOL_CAP`, `EUNKNOWN_TOOL`, `ENOENT`), the `list_files` skip-dir behavior, the `search_files` path-filter and bad-regex paths, the `write_file` overwrite + nested-directory creation, and the `fileReadMaxBytes` cap override.
