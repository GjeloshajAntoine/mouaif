...
- An append also **skips a row whose node is already on screen** under the same `transcriptRowKey`. This is the ordinary live-follower completion path: the streamed bubble was stamped with the row's `seq`, but the live reply is not in `state.messages`, so the merge is a pure append and this cheap path — not `reconcileTranscriptRows` — is chosen. Without the guard the persisted row is drawn as a second bubble beside the live one. Regression case: `scripts/test-transcript-append-keyed-row.js`.
