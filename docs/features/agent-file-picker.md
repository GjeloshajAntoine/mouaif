...
- Styles live in `src/web/src/settings.css` (`.afp__*` overlay/rows + `.settings-project__afn-row` layout). The modal mirrors the file-editor overlay: full-screen sheet on mobile, centered dialog on ≥720 px, same `fe__fade-in` animation and safe-area behavior.
