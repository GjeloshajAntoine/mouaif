...
API-key requests need no caching beta header because prompt caching is generally available. OAuth requests retain their required `anthropic-beta: oauth-2025-04-20` header. The system block is sent as an **array** because a plain string silently drops `cache_control`.
...
- **API-key and OAuth models.** Prompt caching is generally available, so both authentication modes carry `cache_control` markers. OAuth models retain the required `anthropic-beta: oauth-2025-04-20` header; API-key requests do not need a caching beta header.
