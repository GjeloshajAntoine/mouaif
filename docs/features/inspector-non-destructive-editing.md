...
- **An edit that ends where it started leaves no trace.** Typing `16px`, then `20px`, then back to `16px` removes the entry, leaving nothing to undo — the list never offers to reverse a change that is not there.
- **The receipt is capped at 20 entries** (oldest dropped) so a long session cannot grow the strip without bound.
- **A failed revert reports on the panel’s error line** and leaves the entry in place, so the change is not lost from the list just because the write failed.
...
- **Wiring:** the `Receipt` component and the `undoEntry` / `undoOne` / `undoAll` handlers live in [`frontend/src/components/inspector/StylesPanel.jsx`](../../frontend/src/components/inspector/StylesPanel.jsx). The previous value is read from the model **before** the write, because after it the value is gone.
