...
- **Model** uses the same project + live-catalog union, two-line trigger, provider chips, grouped rows, search, and phone viewport sheet as the chat top bar. "Inherit chat model" clears both the saved model and provider. The union reads every provider's live chat catalog (`/api/ai/models/live`), plus the models already pinned in the project. A model that only a separate catalogue lists (there is none today — the image slice was removed with the image tool, see [decisions §28](../../decisions.md)) would need its own read added here.
