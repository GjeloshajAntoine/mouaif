...
- **Model** uses the same project + live-catalog union, two-line trigger, provider chips, grouped rows, search, and phone viewport sheet as the chat top bar. "Inherit chat model" clears both the saved model and provider. The union reads **both the chat slice and the image slice** of every provider's catalog (`/api/ai/models/live` with `purpose=image`), so an image-only model — OpenRouter's `/images/models` rows, a Gemini Imagen model, none of which appear in the chat list — can be pinned to an agent that draws pictures. A provider with no separate image slice returns its chat list, so the extra read is harmless where it does not apply.
