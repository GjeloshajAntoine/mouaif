...
- Mobile-first: the popover is `position: fixed; top: calc(var(--model-picker-viewport-top, 0px) + 48px + var(--safe-top)); height: calc(100dvh - 48px - var(--safe-top) - var(--model-picker-keyboard-inset, 0px));` at `max-width: 480px` so it is sized to the visible viewport, not the larger layout viewport. While the search input is focused, `visualViewport` resize / scroll events update the top offset and the keyboard inset on iOS; the sheet's own height shrinks by the inset, so its list is the only scroll container and bottom rows stay reachable with the keyboard up (a fixed-height sheet would extend under the keys and lose the last rows). The `index.html` viewport meta uses `interactive-widget=resizes-content` so desktop Chrome resizes the layout viewport for the keyboard instead of overlaying it, which would otherwise leave a dead strip behind the sheet. The sheet uses `overflow: hidden` plus a touch scroll lock, and only `.chat-view__picker-list` is allowed to pan vertically; the list also gets keyboard-inset bottom padding so lower rows can scroll above the keyboard without the whole modal/page moving.
