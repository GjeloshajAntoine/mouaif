...
- Mobile-first: the popover is `position: fixed; top: calc(var(--model-picker-viewport-top, 0px) + 48px + var(--safe-top)); bottom: var(--model-picker-keyboard-inset, 0px);` at `max-width: 480px` so it fills the visible screen with the head still visible. While the search input is focused, `visualViewport` resize / scroll events update the top and bottom offsets on iOS so the entire sheet stays inside the visible viewport and the bottom rows remain selectable. Above 480 px it falls back to a floating popover anchored to the head (`position: absolute; top: calc(100% + 4px); left: 6px; right: 6px;`).
