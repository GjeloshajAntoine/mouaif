...
- **Tool calls use a bounded multi-turn loop.** Native and MCP calls emit `tool_call` / `tool_result`, append the result to the upstream conversation, and request a user-facing answer. The loop stops after `maxToolTurns` (12 by default).
