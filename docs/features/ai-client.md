...
- **Subagent delegated cost surfaces mid-turn.** When a `subagent` call finishes, the streaming layer emits a `usage_update` SSE event with the nested run's cost (`{ cost: { known, total, currency }, source: 'subagent' }`). The chat UI folds the delta into the running header "Total" pill immediately, so the cost grows in real time instead of jumping on the parent's `done`. The parent `done` event later carries the same number inside the final segment's remainder, and the running delta is cleared — nothing is double-counted. See [usage-metrics.md](./usage-metrics.md).
