...
- 2–4 options, each with a label and an optional one-line description
...
- **2–4 options.** The model must provide between 2 and 4 options. Fewer is rejected with `EBADINPUT`; more is rejected with `EBADINPUT`. Duplicate `value`s are rejected — `value` is the canonical answer the model sees, and duplicates would collapse options silently.
...
- Source: `src/tools/ask.js` (new module) — `SPEC` (the model-facing tool spec), `validateArgs(args)`, `buildResult({ choice, extra, options, multiSelect, cancelled })`, `clampExtra(value)`, and the cap constants (`MAX_OPTIONS`, `MAX_QUESTION_CHARS`, `MAX_OPTION_CHARS`, `MAX_EXTRA_CHARS`).
...
- The chat UI (`src/web/src/components/Chat.jsx`) listens for `ask_user_required` events and renders a dedicated card with the question, the options as tap targets (44 px min height), the always-on extra textarea, and the two action buttons. The card scrolls into view and moves focus to the first option. The styling lives in `src/web/src/features.css` under the `.tool-card--ask-user` block.
