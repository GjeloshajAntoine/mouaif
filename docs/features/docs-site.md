...
The landing page (`index.html`) opens with a **See it on a phone** section: three phone-width captures of the running app — the Chats tab, the Settings tab, and the Inspector when it has no debugger attached. The copy states the promise (every screen is built for a 360–430 px viewport first) and the captures are the evidence, so a reader sees the app before the install commands.
...
docs/features/images/landing/
  chats-list.png          # Chats tab, 390 × 520 @2x
  settings.png            # Settings tab, 390 × 520 @2x
  inspector-connect.png   # Inspector with no target, 390 × 520 @2x
```
...
- **Wide screens** — one row. `.shot-row` is a grid with `repeat(auto-fit, minmax(210px, 1fr))`, so three captures fit side by side on a laptop and wrap to fewer columns as the width drops. `align-items: start` keeps the different heights from stretching each other.
...
- The capture keeps its own aspect ratio (`width: 100%`, `height: auto` on `.shot img`), so a shorter capture is a shorter card instead of a letterboxed one. The image has no corner radius because the `.shot` card and its `overflow: hidden` already clip it.

### Refreshing the captures

Capture the running app at `390 × 520` with the viewport emulated (a device-metrics override, so the CSS pixel width really is 390) and a device scale factor of 2, then save over the three PNGs above. Use a page whose Chrome debugger URL on `http://127.0.0.1:9222` is not reachable for `inspector-connect.png` — that state is the panel's own explanation of how to attach, which is what the section is showing. Nothing in the build needs updating when an image is replaced: the paths are hard-coded in `buildLandingPage()` in [scripts/build-docs.js](../../scripts/build-docs.js).
