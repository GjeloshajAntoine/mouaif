...
The landing page (`index.html`) ends its hero action row with a GitHub icon button linking to the repository, so a reader can go straight from the presentation page to the code. The URL is the repository's `repository.url` from [package.json](../../package.json), normalized to a plain `https://github.com/<owner>/<repo>` link; when that is not a GitHub repository the button is simply omitted, so the row never renders a dead anchor. The button is icon-only (the GitHub mark) — the action is named by its `aria-label`/`title` (`View the source on GitHub`), not by visible text, and it opens in a new tab (`target="_blank" rel="noopener noreferrer"`). It is generated in `buildLandingPage()` in [scripts/build-docs.js](../../scripts/build-docs.js) — change `repository.url` in `package.json` and the build picks it up.
