...
The site's tagline is **Mobile Web first** — the same phrase as the README's opening line. On both surfaces it is the first thing a reader meets, so the promise ("built for a phone first") is stated before the feature list:
...
| Landing page (`index.html`) | The hero eyebrow above the `<h1>`, `Mobile Web first — mobile-first AI coding assistant` | `buildLandingPage()` in [scripts/build-docs.js](../../scripts/build-docs.js) |
| Documentation index (`documentation.html`) | The lead paragraph under the `<h1>` | `buildDocumentation()` in [scripts/build-docs.js](../../scripts/build-docs.js) |

The eyebrow copy is generated by the build script, not by a Markdown file: change the phrase there and re-run `npm run docs:publish`, which rebuilds and syncs the committed `docs/` output.

At phone widths the eyebrow wraps to two centred lines. It carries an explicit `line-height: 1.45` (and the hero `h1` a `1.1`) so the uppercase glyphs stay inside their own line box — at the inherited `1.6` leading the wrapped line's strokes reached into the `h1` and crossed it.
