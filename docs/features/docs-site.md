...
On a phone, the navigation links scroll sideways on one row with a faded edge, and a link to a heading lands below the sticky bar. The landing page's **Install and run** block leads with the same `npx mouaif serve --auth` command as Getting started, and the hero command block includes a `--public-origin https://mouaif.example.com` line so the domain-name case (behind an HTTPS reverse proxy) is shown where the other `serve` flags are.
