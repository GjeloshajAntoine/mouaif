...
8. **Keywords for a string value.** A value with no per-kind view (`font-family`, `content`, a font stack) gets one more group — the property's own keywords and the CSS-wide four, which are otherwise unreachable for it:
```text
...
9. **Tapping a chip only rewrites the field.** Apply still commits, so a suggestion, a snap, a sibling value, a keyword and a palette pick are exactly as reversible as anything typed — one property, one undo entry.
