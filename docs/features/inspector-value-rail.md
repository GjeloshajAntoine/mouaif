...
- **Pure model:** [`frontend/src/components/inspector/valueRail.js`](../../frontend/src/components/inspector/valueRail.js) exports `railRange`, `familyOf`, `valueToRatio`, `ratioToValue`, `quantize`, `snapStep`, `nudge`, `tickValues`, `majorValues`, `railTicks`, `railLabel`, `railWritable`, `isLogRange`, `fromUnit`, `LOG_RATIO`, `MAX_TICKS`, `MAX_MAJOR` and `unitEquivalent` (the footer's `= 0.875rem`, taken from `unitOptions`' own answer so the readout can only show a conversion a chip in that row would really write). It consumes the scale shape the value index already produces (`{ unit, values, step, decimals }`), so the rail costs no extra page read.
