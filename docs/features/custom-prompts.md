...
Presets are **additive**: a chat that already inherits all project tools keeps them (the preset never restricts a chat to just its listed tools), and the project's Off/Ask/Allow gate for each tool stays authoritative — a tool the project turned `off` remains off even if a preset lists it. Because a preset can only ever *grant* tools, switching **Chat preset** on starts from the all-on baseline: every tool row is checked, and unchecking one records the first explicit selection from there. Re-checking every row collapses back to that baseline. A preset can never mean "no tools at all" — a chat already has every tool, so the schema has no such state (an empty `tools` list normalizes to "absent"). The project's master switches (`agentFiles: false`, `skills: false`) lock the matching preset group off, just like the chat's ToolPopup does; the preset cannot override a project lock. Turning the preset off clears `preset` from the prompt record. The prompt list shows a short `preset: …` badge so the attachment is visible at a glance.
