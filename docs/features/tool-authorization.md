...
- The Settings UI lives in `src/web/src/components/SettingsProject.jsx` under the **Tools** tree. Each tool row is one line: title, a one-line note (which reads "Hidden from the model — costs no tokens." in `off` mode), and a segmented **Off / Ask / Allow** control (`toolModeSegs`). The allowlist editor is a `<details>` disclosure shown only in ask mode; entering patterns writes `mode: "allowlist"` and clearing them flips back to `ask`, so the persisted file and the UI never disagree. Picking **Allow** clears the stored patterns. `ask_user` stays binary (`off` / `ask`). MCP authorization renders through the same tree everywhere the tree appears: the chat tools card and the project settings tree both show an **MCP default** row (the project's shared gate, with its Auto-approve list) plus one **Off / Ask / Allow** segment per configured MCP server (the per-server override — the segment shows the effective mode and gains a ↺ reset when an override is set). The shared control lives in `McpAuthSeg` / `mcpEffective` in [src/web/src/components/settings/toolAuth.js](../../src/web/src/components/settings/toolAuth.js), wired into the chat card by [src/web/src/components/chat/cards.js](../../src/web/src/components/chat/cards.js) and into settings by `SettingsProject.jsx`. The server edit view (`SettingsMcpEditView`) adds the per-tool layer as an **Inherit / Off / Ask / Allow** select under "Discovered tools", and the app MCP page ([src/web/src/components/SettingsMcp.jsx](../../src/web/src/components/SettingsMcp.jsx)) carries the app-level **Default permission** row. Project rows write `{ projectDir, mcp: { mode?, servers?, tools? } }`; the app row writes `{ scope: 'app', mcp: { mode?, allowlist? } }`.
