...
- Mobile-first layout: the `tool_call` and `tool_result` blocks render as monospaced cards with a 13 px monospace font. Shell result cards auto-expand so the terminal output is visible inline (no extra tap needed). Long output is capped at 75 % of the viewport height with internal scroll, preserving the conversation flow. Errors always expand too.
