...
- A viewport-locked, responsive shell for installed mode. `html`/`body` are locked to the viewport (`height: 100%; overflow: hidden; overscroll-behavior: none`) so the document can never scroll or rubber-band, and the app root (`#app`) is fixed to all four viewport edges with `position: fixed; inset: 0` — one box iOS Safari sizes correctly in both the browser and an installed standalone PWA, so no per-`display-mode` viewport-unit override is needed. Individual views own any necessary scrolling, preventing double-scroll or horizontal overflow on narrow screens. Bottom controls use opaque fallbacks and paint through the home-indicator safe area so the inset remains protected without appearing as a detached blank margin.
