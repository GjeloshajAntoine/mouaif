...
- A viewport-locked, responsive shell for installed mode. The document stays non-scrolling and the app root is fixed to all four viewport edges. Installed standalone mode uses the large viewport (`100lvh`) because it has no browser toolbar, avoiding stale or reduced visual-viewport sizing in iOS PWAs. Individual views own any necessary scrolling, preventing rubber-banding or horizontal overflow on narrow screens. Bottom controls use opaque fallbacks and paint through the home-indicator safe area so the inset remains protected without appearing as a detached blank margin.
