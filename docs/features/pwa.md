...
- **Update flow.** A first installation activates immediately because there is no old app bundle to protect. Later SW updates remain waiting and the page shows a "A new version is ready." banner with a Reload button. Tapping it posts `{ type: 'SKIP_WAITING' }` to that worker; the worker activates and the `controllerchange` listener reloads once. Existing sessions never reload merely because an update finished installing.
