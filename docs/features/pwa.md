...
- A viewport-locked, responsive shell for installed mode. The document stays non-scrolling and the app root is fixed to all four viewport edges, avoiding stale or reduced `100dvh` sizing in installed iOS PWAs. Individual views own any necessary scrolling, preventing rubber-banding or horizontal overflow on narrow screens. Bottom controls paint through the home-indicator safe area so the inset remains protected without appearing as a detached blank margin.
