...
- **Update flow.** A first installation activates immediately because there is no old app bundle to protect. Later SW updates remain waiting and the page shows an "A new version is ready." banner as soon as installation finishes, without requiring a manual page reload. Registration observes an update already in progress as well as future `updatefound` events, then performs an explicit update check to cover browsers that finish their automatic check before listeners are attached. While the app remains visible it checks the no-cache worker script once per minute, so a long-running standalone session discovers deployments without a navigation; returning to a hidden app also triggers a check. The page retains the installed worker reference because some engines do not expose it through `registration.waiting` immediately. Tapping Reload posts `{ type: 'SKIP_WAITING' }` to that worker; the worker activates and the `controllerchange` listener reloads once (including on a first session, where the page loaded uncontrolled — see the next two bullets). Existing sessions never reload merely because an update finished installing.
- **A first install does not reload the page it claims.** The worker's activate handler calls `clients.claim()`, which takes control of the page already on screen and fires `controllerchange` on a window that loaded UNCONTROLLED. The handler used to reload on every `controllerchange`, so the first visit to an origin installed the worker, painted the app, and then threw that page away and loaded it again — a white flash, a second document load, and a second round of chat requests on what looked like a normal first open. `frontend/src/sw-controller-reload.js` decides instead: for a controller change the user did not ask for, reload only when the page was **already controlled when it loaded** and still is, and never for a page the first install just claimed; a reload the user *did* ask for is covered by the next bullet. Sampling the flag at load time is what makes the distinction — read when the event fires, `navigator.serviceWorker.controller` has already been flipped by the claim, which is exactly how the two cases became indistinguishable.
- **Accepting an update reloads even on a first session.** The load-time flag answers "did a controller already own this document?", which is the wrong question for a reload the user just asked for. On a first session the page loaded uncontrolled, so that flag is false for the document's whole life — a deploy landing mid-session showed "A new version is ready.", and tapping Reload posted `SKIP_WAITING` for a `controllerchange` the flag then refused: the banner disappeared and the session kept running the old bundle with no way to pick up the new one. `applyUpdate()` now records the intent in `acceptedUpdate` before it posts `SKIP_WAITING`, and the predicate reloads on `acceptedUpdate && hasController` regardless of how the page loaded. Intent also covers the case where an engine delivers the `controllerchange` before the `SKIP_WAITING` post resolves.
