...
- **A controller change the user did not ask for never reloads the page.** `clients.claim()` runs on *every* activation, so it also fires `controllerchange` on windows that loaded CONTROLLED: a deploy another tab accepted, or a worker evicted and then re-registered by a later visit, claims every open client. An earlier rule keyed the decision on "was this page controlled when it loaded?", which answered *yes* for those tabs and reloaded them anyway — a long chat tab reloading at a moment tied to deploys and worker churn rather than to anything the user did. `frontend/src/sw-controller-reload.js` now reads intent instead of control state: a controller change reloads only when `acceptedUpdate` is set (the user tapped Reload on the banner) and a controller owns the page. A claim nobody requested is something that happened *to* the page, so it is left alone; the next natural navigation picks up the new bundle.
...
- **No flash before the stylesheet.** `frontend/index.html` carries an inline rule in `<head>` that paints the app's full `html` background before the bundle's stylesheet arrives. The document used to have no background of its own — every surface came from the `@import`-ed index CSS — and that external stylesheet is requested before the module script runs, so between the document commit and the arrival of the CSS the page painted the browser's *default canvas* instead of the app. Measured on a headless Chrome at 390 × 844: a document with the `color-scheme: dark` meta and no stylesheet paints `#121212`, and with no hint at all it paints `#fff`, against an app whose `--bg` is `#0a0d12 / rgb(10, 13, 18)`. That gap is one or more frames of a lighter, near-grey full-screen fill between a reload and the first render — reported as "grey flashes with page reload".
  - The inline rule is in the document, so it applies at the first paint, and it reproduces the bundle's `html` background *exactly* — the same `radial-gradient(...), var(--bg)` stack from [frontend/src/base.css](../../frontend/src/base.css) with `--bg` resolved to its literal `#0a0d12`. The two rules set the same `background` property at the same specificity, so their *order* is what matters as well: the inline block sits before the `<link rel="stylesheet">` Vite appends to the end of `<head>`, and once the bundle's rule lands it overrides the inline rule with the identical pixel stack — a no-op repaint instead of a step. An earlier rule set only the solid base colour (`background: #0a0d12`); that fixed the grey flash but introduced a second symptom where the same base colour plus the accent glow repainted visibly once the bundle's CSS landed. Matching the two values ends both at once. [scripts/test-index-first-paint.mjs](../../scripts/test-index-first-paint.mjs) pins all three facts — the inline rule exists in `<head>`, its background stack equals the bundle's `html` background, and in the built `dist/index.html` it still precedes the bundle's stylesheet and the gradient survives minification — so a drift in any of them fails the test.
