...
- **Continuous refresh** — while Preview is visible, a low-resolution CDP `Page.startScreencast` supplies event-driven repaint notifications. Each frame is acknowledged after its full-page screenshot and captures are coalesced to at most ~10 per second, so Chrome applies natural backpressure while animations, typing, hover states, and DOM mutations remain live without a busy polling loop. Lifecycle captures (`Page.frameNavigated` and `Page.frameStoppedLoading`) plus a slow 3 s safety retry cover older targets that do not support screencasting.
- **Capture quality** — previews use lossless PNG at the inspected page's native capture resolution, avoiding JPEG artifacts around small text, colored edges, and fine UI details. The panel, full-screen view, and Draft Craft annotation all use the same lossless image; no setting is needed. Viewport sizing, tap coordinates, and the ~10 fps capture limit are unchanged. PNG captures can use more bandwidth on photo-heavy pages, so the existing single-capture backpressure still limits work on slower connections. The discarded screencast signal stream stays low-resolution JPEG (`quality: 20`, 320×320) because it is only a change detector, not the rendered image.
...
The inspector header icon buttons are **labelled, not glyph-only**. The panel-head controls (full screen, Draft Craft, refresh, type into page, hide/show) and the Styles panel header (clear, refresh, pick/stop) each carry a small label under their glyph via the shared `.icon-btn--labeled` variant — the same icon-over-label pattern as the panel chips. The labels stay compact (small type, tight padding, fixed-height `--tap` button) so both header rows fit on a **single line** down to the 360 px minimum viewport; the preview-size dropdown is width-capped and ellipsizes, and the Styles element label ellipsizes instead of wrapping. Every button keeps its `aria-label` + `title` for assistive tech. Only the nav-row back/reload glyphs stay icon-only, since their meanings are conventional and space is at a premium next to the URL field.
