...
- **Capture quality** — full-page screenshots are captured as high-quality JPEG (`quality: 90`) so text and fine detail stay readable when you pan through a tall page. The discarded screencast signal stream stays low-resolution (`quality: 20`, 320×320) because it is only a change detector, not the rendered image.
