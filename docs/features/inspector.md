...
- Treats a capture that resolves with **no image data** as a transient skip, not an error. Chrome returns an empty `Page.captureScreenshot` result on the very first call right after `Page.enable` and again right after a navigation starts; in both cases the fallback poll retries a moment later. Skipping (rather than flipping the note to `screenshot failed`) keeps the previous frame on screen and the status at `live`/`capturing…` until a real frame lands.
