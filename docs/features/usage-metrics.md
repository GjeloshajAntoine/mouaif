...
- **Subagents are included.** When the model delegates work to the native `subagent` tool, the nested model call's cost is resolved with the subagent's own model and added to the parent chat turn's final remainder cost. This keeps the header, chat list, and project total aligned with provider billing even when the subagent uses a different model. Direct `@agent` runs are persisted as assistant messages with the same usage/cost metadata and update the header Total immediately.
