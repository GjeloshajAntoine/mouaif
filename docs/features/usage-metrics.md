...
- `37 tok/s` — completion tokens per second for that reply, averaged over its own streaming window. On a tool turn, every reply segment (the text before each tool call, and the final answer) shows its own speed.

The `tok/s` line updates **live** as the assistant's deltas arrive: it climbs from `0` while the upstream warms up, follows the throughput as tokens flow, and freezes on the final value when `done` is received. The speed is saved with each message, so a reopened chat shows the same value. If the stream ends in `error` (e.g. `EUPSTREAM`, `EABORTED`, `EENETWORK`), the counter freezes at the last value and the cost line shows `--`.
...
- **Counter formula.** `tok/s = completionTokens / streamingMs`, measured **per round**. The window starts on the first `message` **or `reasoning`** delta of a round and stops at that round's end (`assistant_turn_end` before a tool call, or `done`; the last delta before `error` / `EABORTED`). Tool runs between rounds are never counted as streaming time, and each round's window is paired only with that round's own `completionTokens` — never the turn total. Sub-second windows are allowed. Thinking deltas feed the counter for the same reason the window spans them: upstream `completionTokens` include thinking tokens, so dividing them by the answer-only window would over-report tok/s by the think/answer ratio on reasoning-heavy turns.
...
- **Persisted with the message.** Each assistant row (tool-round segments and the final message) stores its round's `streamingMs` next to its `usage`, so the saved speed is `usage.completionTokens / streamingMs`. The `assistant_turn_end` and `done` SSE frames carry the same `streamingMs`; `done` also carries `roundCompletionTokens` (the final round's output) because its `usage` is the turn total.
- **Usage block.** The full `usage` object returned by the AI client is stored on the assistant message record and is also written to the [trace file](./trace.md) as a `done` event. Re-opening a chat shows the same numbers that were visible at the time the message was produced.
