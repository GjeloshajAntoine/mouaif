...
- **Subagents are included — and reflected live.** When the model delegates work to the native `subagent` tool, the nested model call's cost is resolved with the subagent's own model and added to the parent chat turn's final remainder cost. This keeps the header, chat list, and project total aligned with provider billing even when the subagent uses a different model. The streaming layer emits a `usage_update` SSE event with the nested run's cost as soon as the subagent finishes, so the header "Total" pill grows in real time instead of jumping on the parent's `done`. The `done` event later folds the same number into the final segment's remainder and the running delta is cleared; nothing is double-counted on reload. Direct `@agent` runs are persisted as assistant messages with the same usage/cost metadata and update the header Total immediately.
