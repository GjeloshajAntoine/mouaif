...
- **Attribution headers.** Every request carries `HTTP-Referer: https://mouaif.local` and `X-Title: <app name>` headers, per [OpenRouter's docs](https://openrouter.ai/docs/api-reference/overview). The X-Title identifies the app on the public leaderboard and is sourced from `app.openRouter.appName` at request time; the shipped default is `mouaif`. Both headers do not send PII. They can be overridden per-request by `model.headers` for testing.
- **App name on the provider form.** The Settings → Providers form (OpenRouter) shows an **App name** field with its own **Save app name** button, separate from the provider Save. The value is persisted in app settings (`app.openRouter.appName`) and read by the AI client at request time when building the OpenRouter `X-Title` header. Blank / whitespace / null falls back to the shipped `mouaif` default. The value is clamped to 64 chars before being sent. Decoupling the field from the provider Save lets the user set the app name, then sign in, then save the provider record — the natural order.
...
- **Source: [src/ai.js](../../src/ai.js).** The provider is registered as a single entry in `ENDPOINTS` (with `baseUrl`, `chatPath`, `authHeader`, and `staticHeaders`) and reuses the existing `buildOpenAIRequest` and `parseOpenAISSE`. No new builder or parser was needed. `buildOpenAIRequest` calls `resolveOpenRouterStaticHeaders()` for the `openrouter` provider, which reads `app.openRouter.appName` from the app store at request time and substitutes it for the shipped `mouaif` default in the `X-Title` header. Blank / null falls back to the default. The substitution is read-only (no mutation of `ENDPOINTS`) and clamps the user-supplied name to 64 chars.
