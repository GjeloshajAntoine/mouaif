...
- **Attribution headers.** Every request carries `HTTP-Referer: <url>` and `X-OpenRouter-Title: <app name>` (plus the deprecated alias `X-Title: <app name>`), per [OpenRouter's docs](https://openrouter.ai/docs/api-reference/overview). `X-OpenRouter-Title` is the canonical attribution header on current OpenRouter releases (the older `X-Title` is silently dropped on the SSO/leaderboard path, so we send both). `HTTP-Referer` is the URL OpenRouter scrapes for `og:*` tags when building the public-leaderboard entry; the shipped default `https://mouaif.local` is a fictitious domain nobody owns, so the app name is not visible on the public leaderboard until the user configures a real URL they own via `app.openRouter.httpReferer`. Both headers do not send PII. They can be overridden per-request by `model.headers` for testing.
- **App name on the provider form.** The Settings → Providers form (OpenRouter) shows an **App name** field with its own **Save app name** button, separate from the provider Save. The value is persisted in app settings (`app.openRouter.appName`) and read by the AI client at request time when building the OpenRouter `X-OpenRouter-Title` and `X-Title` headers. Blank / whitespace / null falls back to the shipped `mouaif` default on both header names. The value is clamped to 64 chars before being sent. Decoupling the field from the provider Save lets the user set the app name, then sign in, then save the provider record — the natural order.
- **HTTP-Referer URL on the provider form.** The Settings → Providers form (OpenRouter) shows an **HTTP-Referer URL** field with its own **Save URL** button. The value is persisted in app settings (`app.openRouter.httpReferer`) and read by the AI client at request time when building the OpenRouter `HTTP-Referer` header. The shipped default is `https://mouaif.local`; without an owned domain the app name is invisible on the public leaderboard even when `X-OpenRouter-Title` is set. The value must be an absolute `http(s)` URL — non-http schemes (`ftp:`, `file:`, ...) and unparseable strings are silently dropped back to the shipped default rather than poisoning the request.
...
- **Source: [src/ai.js](../../src/ai.js).** The provider is registered as a single entry in `ENDPOINTS` (with `baseUrl`, `chatPath`, `authHeader`, and `staticHeaders`) and reuses the existing `buildOpenAIRequest` and `parseOpenAISSE`. No new builder or parser was needed. `buildOpenAIRequest` calls `resolveOpenRouterStaticHeaders()` for the `openrouter` provider, which reads `app.openRouter.appName` and `app.openRouter.httpReferer` from the app store at request time and substitutes them for the shipped `mouaif` / `https://mouaif.local` defaults in the `X-OpenRouter-Title`, `X-Title`, and `HTTP-Referer` headers. Blank / null falls back to the defaults. The substitution is read-only (no mutation of `ENDPOINTS`) and clamps the user-supplied app name to 64 chars. A non-http(s) `httpReferer` value is silently dropped back to the shipped default.
...
- **Tests: [scripts/test-openrouter.js](../../scripts/test-openrouter.js).** 60 assertions: `authProviderFor` mapping, request URL, Authorization header, HTTP-Referer + X-OpenRouter-Title + X-Title static headers, no Copilot Editor-Version leakage, body shape, end-to-end `streamChat` round-trip with a mocked fetch (parses the SSE, emits a `message` event, emits a `done` event with usage), `SUPPORTED_PROVIDERS` membership, `buildAuthorizeUrl` URL shape, `code_challenge = base64url(sha256(verifier))` invariant, `accountForKey` short-label helper, `exchangeAuthorizationCode` request body, the registered `exchange` / `refresh` shape, and the request-time `app.openRouter.{appName,httpReferer}` resolution (override, blank-fallback, 64-char clamp, valid http(s) URL pass-through, non-http(s) drop-back, invalid-string drop-back).
