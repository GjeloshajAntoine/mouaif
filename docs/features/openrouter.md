...
- **App name on sign-in.** The Settings → Providers form (OpenRouter + OAuth) shows an **App name** field above the Sign in button. The value the user types (e.g. "Work laptop", "Personal") is forwarded to `POST /api/auth/sign-in/openrouter` as `appName`, stored on the pending OAuth record, and used by the registered exchange function as the `account` slot. This lets a user who signs in twice (e.g. on a work laptop and a personal desktop) give each sign-in a recognisable label in the OAuth account picker; blank falls back to the auto-generated key prefix in `accountForKey(key)`. Clamped to 64 chars. Per-session only — not saved to the provider record — so each new sign-in can use a different name.
...
- **Account naming.** Each sign-in produces a separate row in the OAuth account picker. The label is the user-supplied **App name** from the Settings form (clamped to 64 chars) when set; otherwise the first 16 characters of the issued key (e.g. `sk-or-v1-mock-12`). The user-supplied name is recognisable on a phone-sized picker without leaking the full secret.
