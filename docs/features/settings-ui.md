...
| `#/settings/project[?projectDir=<abs path>]` | `SettingsProjectView` | Per-project overrides (prompt style, tool authorization, agent files, agents) + a "This chat" group for the chat the user came from (`?chatId=…`). Agent files default to on unless the project explicitly sets `agentFiles: false`; chat-level toggles can opt out, but cannot bypass that project lock. Raw `<projectDir>/.mouaif.json` editor and resolved view live in a collapsed Advanced section. Seeds the directory from `?projectDir=` or the active project and auto-loads. |
| `#/settings/project/technical[?projectDir=<abs path>][&chatId=<id>]` | `SettingsProjectView` (`page: 'technical'`) | **Technical details** — raw `.mouaif.json` editor, resolved settings, chat import, and (when `chatId` is present) the current chat's trace toggle, trace export, and delete-chat actions. |
...
The project view's groups are scoped on purpose: a **Chat defaults** group holds settings that live in `.mouaif.json` and apply to chats in this project (prompt style today; anything left on its default follows the app-level value). Per-chat actions are never mixed into a project group — a per-chat action next to a project setting reads as "this writes `.mouaif.json`", which it does not. The trace toggle, trace export, delete-chat, and chat import all live on the **Technical details** page instead. Reach it from the chat's settings button → the **Technical details** link at the bottom of project settings, which carries the `chatId`; the project's "More settings → Import chats" entry also reaches it (project-wide, no chat needed). The page shows the chat-scoped trace/export/delete controls only when a `chatId` is on the route; without one it shows just the raw `.mouaif.json` editor, the resolved settings, and the import action.
