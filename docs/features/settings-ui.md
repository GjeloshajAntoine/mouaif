...
| `#/settings/project[?projectDir=<abs path>]` | `SettingsProjectView` | Per-project overrides (prompt style, tool authorization, agent-file names, agents) + a "This chat" group for the chat the user came from (`?chatId=…`). The Agent files section is an item-list editor for `agentFileNames`; chat-level controls own the on/off state. Raw `<projectDir>/.mouaif.json` editor and resolved view live in a collapsed Advanced section. Seeds the directory from `?projectDir=` or the active project and auto-loads. |
...
Project settings use a regular, single-column form: a quiet path and scope introduction followed by consistently titled sections (`General`, `Current chat`, `Tools`, `Agent files`, `Agents`, and `More settings`). Controls span the available mobile width, descriptions use plain language, and every existing option remains available. The Agent files section uses tappable list rows with **Add**, **Pick**, **Remove**, and **Use defaults** actions instead of a multiline textarea or project-wide disable switch. Advanced raw JSON stays collapsed at the bottom.
