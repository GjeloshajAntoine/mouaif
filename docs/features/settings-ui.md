...
The project view's groups are scoped on purpose: a **Chat defaults** group holds settings that live in `.mouaif.json` and apply to chats in this project (prompt style today; anything left on its default follows the app-level value). Per-chat actions are never mixed into a project group — a per-chat action next to a project setting reads as "this writes `.mouaif.json`", which it does not. The trace toggle, trace export, delete-chat, and chat import all live on the **Technical details** page instead, which the chat's gear button and the project's "More settings → Import chats" entry both reach. That page carries a `chatId` on the route to show the chat-scoped trace/export/delete controls; without a `chatId` it shows only the raw `.mouaif.json` editor, the resolved settings, and the import action.
