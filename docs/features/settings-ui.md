...
**GitHub Copilot sign-in.** The Copilot provider form signs in with a GitHub one-time code (**Sign in with GitHub code**), which needs no callback URL and uses the shipped public client. An optional **Advanced: custom GitHub OAuth app** disclosure holds the per-install `client_id` (stored in app settings under `githubCopilot.clientId`; blank = shipped default) with its own "Save client ID" action and a **Sign in with browser redirect** fallback for the web flow. The legacy `#/settings/copilot` screen is a redirect to this form. See [GitHub Copilot](github-copilot.md).
