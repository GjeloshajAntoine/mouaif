...
- **Application** — global, project-independent: `Chat defaults`, `Model pricing`, `About & reset`. (The GitHub Copilot OAuth-app `client_id` is **not** a separate card here — it lives inside the Copilot provider form, next to that provider's sign-in, so all Copilot auth is in one place.)
