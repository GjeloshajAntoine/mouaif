...
- **Transcript area** — displays conversational messages with markdown support, expandable system prompt details, inline tool run cards with live execution status, and per-message usage metrics. Assistant rows (prose bubbles, thinking cards, tool cards, the system prompt card) are capped and centred so their left and right gutters match; user bubbles stay right-aligned so the two sides of the conversation remain telling apart.
