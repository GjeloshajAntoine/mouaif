...
- **Model** — full-width `<select>` on the head's second row, paired with a refresh button (↻) flush right. The picker is populated from the union of `GET /api/ai/models?projectDir=…` (the project's hand-typed `models` array) and `GET /api/ai/models/live?provider=<id>` (the upstream's live catalog, cached 1h on the server — see [docs/decisions.md §20](../decisions.md)). The list is deduped by id with the project entries winning. A brand-new chat fires one auto-refresh on load so the picker opens with the full provider catalog even when the project has no models yet. The refresh button re-fetches the live list for the current provider, restoring the previous selection if still present. The current value is read on send.
