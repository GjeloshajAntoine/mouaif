...
- **⚙ Settings** — opens the settings popover. Inside:
  - **Prompt size** — select populated from `GET /api/prompt-profiles` (the same list the server uses to resolve the active system prompt). `PATCH` with `{ promptSize }` on change. A one-line description under the picker (`.chat-view__settings-hint`) shows what the active profile does; the meta line under the title (`<label> · trace on/off`) updates on success. The same value is read on the server when the chat is opened, so the next message uses the new profile. See [Prompt-size profiles](./prompt-profiles.md).
