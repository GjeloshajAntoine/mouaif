...
- **Prompt-size switch + tool preview, only at creation time.** A segmented `S` / `M` / `L` control (`.chat-view__switch`) sits on its own full-width row under the model select, with a tool-declaration preview (`.chat-view__toolprev`) directly beneath it. Both are shown **only while the chat has no messages yet** — the prompt size is a creation-time choice, not a permanent fixture. A tap PATCHes `{ promptSize }`, updates the System bubble, and re-fetches the preview. The preview is fed by `GET /api/chats/:id/tool-preview` and shows the concrete effect of the profile on the tool budget: how many tools ride, and whether each carries its full parameter schema (average / extensive) or just a name + one-line description with the schema stripped (very-small). As soon as the first message is sent, `updateSetupVisibility()` hides both controls for the life of the chat — the prompt size is fixed from that point. There is no prompt-size control in the ⚙ popover anymore.
