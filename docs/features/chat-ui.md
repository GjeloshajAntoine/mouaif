...
- **Prompt-size switch in the head.** A segmented `S` / `M` / `L` control (`.chat-view__switch`) sits on its own full-width row under the model select. It shows the active profile at a glance and sets it in one tap (very-small / average / extensive) — no popover needed. A tap PATCHes `{ promptSize }`, updates the System bubble, and (server-side) changes how much of each tool is declared to the model. The same value is still editable in the ⚙ popover.
...
- **⚙ Settings** — opens the settings popover. Inside:
  - **Prompt size** — select populated from `GET /api/prompt-profiles` (the same list the server uses to resolve the active system prompt). `PATCH` with `{ promptSize }` on change. A one-line description under the picker (`.chat-view__settings-hint`) shows what the active profile does, and the system-prompt card at the top of the transcript is refreshed so the change is immediately visible. The prompt-size is **not** echoed on the meta line anymore. See [Prompt-size profiles](./prompt-profiles.md).
...
All `PATCH`-style controls share a single `updateChat(patch)` helper. On success it overwrites the local `chat` record with the server's response, which is the single source of truth for the head's title, the (trace-only) meta line, and the popover's controls. The prompt-size and custom-prompt handlers additionally re-fetch `GET /api/chats/:id/system-prompt` and re-render the system-prompt card. On failure the status line shows the HTTP code and nothing on the page changes.
