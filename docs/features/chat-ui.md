...
- **System prompt is the first message.** The resolved prompt-size profile (plus any custom prompt) is rendered as a collapsible `.chat-msg--system` card pinned to the top of the transcript, fed by `GET /api/chats/:id/system-prompt`. Collapsed by default to a single line (`System prompt · <profile label>[ + <custom prompt title>]`); tap to expand the full text. It refreshes in place when the prompt-size or custom prompt is changed in the ⚙ popover. This is what "the prompt appears as the first message" means — the profile is a visible, on-demand message, not a permanent picker readout.
