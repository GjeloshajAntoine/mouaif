...
- **Tools card** — a collapsible card below the system-prompt message groups every available tool (Shell, Subagent, Ask user, File tools, an **MCP default** row, and one row per MCP server). Each group row carries a visibility checkbox (on the left) and a **Off / Ask / Allow authorization segment** (on the right), matching the same control pattern as the project settings page. The segment writes through `PUT /api/tools/authorization` immediately on tap; the card re-renders in place to reflect the new mode. MCP rows write the layered `.mcp.json` authorization: the **MCP default** row is the project's shared gate (`mcp.mode`), and each server row is that server's override (`mcp.servers.<slug>`) showing the effective mode, with a ↺ reset when overridden. The per-chat visibility filter (which tools are exposed to the model for this chat) is separate from the project-level authorization mode (who decides whether a call is approved). Both controls live side by side on the same row, with the authorization segment pushed to the right edge via `tool-tree__control`.
...
**On-screen keyboard.** The shell height is `calc(100dvh - var(--kb-inset, 0px))`. `--kb-inset` is published on `<html>` by `frontend/src/viewport.js`, which watches `window.visualViewport` and measures the vertical space covered by the soft keyboard. iOS Safari and iOS standalone PWAs ignore the Chromium-only `interactive-widget=resizes-content` viewport directive — their *layout* viewport (and therefore `100dvh`) never shrinks when the keyboard opens, only the visual viewport does — so without this the composer stayed pinned to the bottom of the layout viewport: hidden behind the keyboard in the PWA, or forcing the browser to scroll the page and leave an empty margin under the composer. On Android/Chrome the inset computes to `0` (the meta directive already shrinks `vv`), so the subtraction is a no-op there. The model picker uses the same `visualViewport`-based technique bound to its own sheet (see [`model-picker.md`](./model-picker.md)).
