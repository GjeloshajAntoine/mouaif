...
While a chat is loading, the transcript shows a **loading placeholder** — a centred ring and one sentence — instead of an empty box. The chat load is a single batched round trip (the chat record, the message window, and the prompt/tool/agent catalogues), and until it lands there is no row to paint; on a slow link or the first open after a cold start the blank area read as a broken view. The placeholder is mounted the moment the load starts and is replaced by the first rebuild. The two failure paths swap it for a plain sentence with no ring — *This chat could not be found…* on a `404`, *Could not load this chat…* when the batch throws — so a request that never answered leaves a visible reason beside the status line. See [Chat loading state](./chat-loading-state.md).
