...
When no model is configured, `runImageTool` falls back to the project's own image-capable model records and the connected providers' live catalogs (see the REST surface below).
...
- **Frontend.** [frontend/src/imageGeneration.js](../../frontend/src/imageGeneration.js) holds the pure helpers (load models, generate, read image blocks out of a result, data URL, kind label). The card renderer is `renderImageGenToolResult` in [frontend/src/components/chat/toolRender.js](../../frontend/src/components/chat/toolRender.js), which consumes the image-block/data-URL helpers; a result that reached the UI as plain text (a replayed row) says `Image bytes are not part of this result.` instead of painting an empty frame. The project-settings screen no longer embeds a model picker — the row is only the Off/Ask/Allow segment — so the `loadImageModels`/`kindLabel` helpers are consumed by the REST surface and other callers rather than that screen.
- **Mobile-first.** The thumbnail is capped at 220 px by the existing `.tool-card__image` rule, the lightbox is `position: fixed; inset: 0` with `object-fit: contain` so a wide picture fits a 360 px viewport, and each thumbnail is wrapped in a button (≥ 44 px tap target).
