...
- **Results are ranked**, best first: exact name, name prefix, name word start (after `/ _ . - :` or a camelCase hump), name substring, path prefix, path substring, other search text (tags, descriptions), then a **fuzzy** subsequence of the name or path — `@atmjs` finds `atMention.js`. Ties keep the catalog order.
- The matched characters of each name are **highlighted** in the accent color so it is clear why a row matched.
- **Matching files always appear first**, ahead of matching agents, actions, tools, MCP tools, and the model. Relevance order is preserved within the file and non-file groups. Fuzzy-only matches come after every real (prefix/substring) match, so a loose file subsequence never buries an exact tool or agent name.
- A query that matches nothing shows **No matches for @…** instead of silently closing the popup.
...
- Navigate with **Arrow Down/Up** (wrapping from the last row back to the first), select with **Enter** or **Tab**, dismiss with **Escape** (also when nothing matches) or click outside. For a complete custom action, press Enter to run it directly from the popup; an exact action ID takes precedence over similarly named file suggestions and works even when the composer normally uses Enter for a newline.
- Selecting replaces the **whole** `@` token under the caret, including characters after it — editing `@src/fo|o.js` and picking a file does not leave a stray `o.js`, and no double space is left behind.
