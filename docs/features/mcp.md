...
Each list row carries its lifecycle controls (**Start** / **Restart**, **Stop**, **↻** refresh on a ready server). Feedback for those actions is **inline on the row**: an in-flight action shows a neutral "starting…" / "refreshing…" line under the buttons, and a failure shows the typed error there with a next step ("tap the row to check the command/URL, then try again") — errors never land in a page-level status line away from the row that caused them. A row whose server is **off** explains itself: the Start button is disabled with a tooltip, and the row shows "Server is off — the model does not see its tools. Tap the row to edit and re-enable it." — a disabled control never appears without a reason. A server-side `errored` status renders the same slot with the typed code and a retry hint. In the chat tool tree the same rule applies: a disabled server locks the whole group (group checkbox and leaf checkboxes go inert) and the row states why under it.
