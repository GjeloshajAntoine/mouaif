...
2. Tap **+** to add a server. Fill in:
   ...
   - **Transport** — `Stdio` for a local child process, `HTTP` for an MCP Streamable HTTP endpoint, or `SSE (legacy)` for a server that only speaks the older HTTP+SSE transport.
   ...
   - **HTTP URL** / **SSE URL** — remote only; the Streamable HTTP endpoint (for example `https://example.com/mcp`) or the SSE stream (for example `https://example.com/sse`).
   - **Authentication** — remote only; manual headers or [OAuth](./mcp-oauth.md): browser sign-in with PKCE (automatic registration, a public client ID, or a confidential client ID + secret) or client credentials with no browser. Tokens and secrets stay in the OS keychain. Save, reopen the server, then sign in or connect.
...
- **Tool names are provider-safe.** OpenAI-shaped APIs reject any function name outside `^[a-zA-Z0-9_-]{1,64}$`, and one bad name fails the whole request. A composed name that already fits is used as-is. Otherwise invalid characters become `_`, the name is truncated to fit 64 characters, and a 6-character hash of the raw name is appended so `a.b` and `a_b` stay distinct:

   ```text
   raw:      some.dotted/tool_with_a_rather_long_name_that_keeps_going_and_going
   composed: mcp__probe__some_dotted_tool_with_a_rather_long_name_that_434b86
   ```

   `mcp.callTool` accepts either the raw MCP name or the composed tool part and always sends the server the raw name it advertised. `GET /api/mcp/servers` returns each tool's `composedName`, which the Settings per-tool authorization list and agent tool allowlists use as their key.
...
- **Errors are typed.** Transport failures become `EMCP_TRANSPORT`; JSON-RPC errors become `EMCP_RPC`; a call that outlives its request timeout becomes `EMCP_TIMEOUT` (HTTP 504 on `POST /api/mcp/call`); a call cancelled by Stop becomes `EABORTED`; missing slugs become `EMCP_NOSESSION`; missing tools become `EMCP_NOTFOUND`. The chat UI can branch on `code` without parsing the message.
- **Stop cancels MCP calls.** The chat's abort signal is passed to every MCP `tools/call`. Tapping Stop rejects the in-flight call at once with `EABORTED`, the SDK sends the server `notifications/cancelled` for that request, and the tool loop ends without another model round. The server session stays `ready`. `POST /api/mcp/call` does the same when the HTTP client disconnects.
- **Per-call request timeout.** The default is 60 s. Set `defaultTimeoutMs` and/or `maxTimeoutMs` in the `.mcp.json` `authorization` block — per tool, per server, project-wide, or app-wide, with the same most-specific-first layering as `mode` — to change it. The result is `min(defaultTimeoutMs ?? 60000, maxTimeoutMs ?? 600000)`:

   ```json
   {
      "authorization": {
         "mode": "ask",
         "servers": { "chrome_debug": { "mode": "allow", "defaultTimeoutMs": 120000 } }
      }
   }
   ```

   The timeout is read from the same entry that decides the call's `mode`: the most specific layer that sets a `mode` wins as a whole, so a timeout on an entry with no `mode` is ignored, and fields are not merged across layers. Only values set in the MCP block apply; the generic 30 s tool default used by `shell` does not.
- **Remote MCP: Streamable HTTP or legacy SSE.** Set `transport: "http"` and `url` to a Streamable HTTP endpoint, or `transport: "sse"` for an HTTP+SSE server (MCP 2024-11-05), whose message endpoint must share the stream's origin. Optional headers are sent with every transport request, including SSE message POSTs, and are redacted in API responses.
