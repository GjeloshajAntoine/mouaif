...
- **Settings → This project → MCP servers** (under **More settings**; `#/settings/mcp?projectDir=…`) — the single per-project entry point. It shows the merged server list (app entries first, each marked with an **app** / **project** badge), the per-server **Tool permissions** rows (Off/Ask/Allow + auto-approve list, falling back to the project default), and the project default gate (per-server / per-tool authorization maps live in `.mcp.json`; the project default falls through to the app default). The compact, edge-to-edge server rows match the Agents list, show status and tool count, retain inline lifecycle actions, and link to the server editor (`#/settings/mcp/<id>?projectDir=…`).
...
- **Settings → App defaults → MCP servers** (`#/settings/mcp`) and **Settings → This project → MCP servers** (`#/settings/mcp?projectDir=…`) both render a single item list of servers (name, scope badge, status, tool count, lifecycle buttons). The list is the only surface — per-server, per-tool, and shared-gate authorization controls are not shown here; MCP permissions are configured per tool from the server editor's **Discovered tools** section (`#/settings/mcp/<id>`).
