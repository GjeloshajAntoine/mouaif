...
Each list row carries its lifecycle controls (**Start** / **Restart**, **Stop**, **↻** refresh on a ready server). A row whose server is **off** explains itself: the Start button is disabled with a tooltip, and the row shows "Server is off — the model does not see its tools. Tap the row to edit and re-enable it." — a disabled control never appears without a reason. In the chat tool tree the same rule applies: a disabled server locks the whole group (group checkbox and leaf checkboxes go inert) and the row states why under it.
