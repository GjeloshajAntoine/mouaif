...
- **Servers auto-start on chat open.** `GET /api/tools/list` awaits `ensureServersRunning(projectDir)`: every configured server that is not already `ready` / `starting` is spawned and re-discovered before the catalog is returned, including servers loaded directly from `.mcp.json` with no existing cache. A failed start is captured as `errored` on that server only — the rest still starts and native tools are still returned. **The catalog is loaded in the background on the chat view:** the web UI issues `/api/tools/list` after the transcript and model picker have already painted (each MCP cold-start spawns a child process and can take seconds), so opening a chat is never blocked on MCP startup. When the catalog resolves, the tools card swaps in place and the ToolPopup picks up the populated tool list; a failed request leaves the empty-catalog card.
