...
- **Full image** — tap the card to open the screenshot in a full-screen viewer (title with a short host sub-line, a **Refresh** button, the Size dropdown, capture time and resolution in the footer, and Open-in-new-tab). On phones (≤ 430px) the viewer's header stacks: the title/host block sits on the first row with the close ✕ mirrored to the top-right, and the Refresh + Size controls drop to a second row. The title block reserves the close-button band (`flex: 0 0 calc(100% - 52px)`) so a long page title never runs under the close button. The close button is offset `8px` from the **head's** top edge, not from the viewport: the overlay already pads out the status-bar safe area, so measuring from the viewport added that inset a second time and dropped the ✕ onto the controls below it on a notched phone.
