...
- **Live (iframe) mode** — the viewer's footer has a **Screenshot / Live** toggle (two 44 px buttons, shown only for `http:` / `https:` URLs). **Live** swaps the screenshot for an interactive, sandboxed `<iframe>` of the same URL. The frame is rendered at the selected Size (Phone 375 × 667 by default) and scaled down to fit the sheet, so the page lays out as it would on that device; the footer shows the frame size and the scale. In Live mode **Refresh** reloads the frame and the Size control resizes it; no screenshot is captured and no authorization is asked. The choice is remembered on the device (`localStorage` key `mouaif.webpreview.viewMode`).
- **Live mode limits** — the frame is loaded by **your browser**, not the server's debug Chrome: `localhost` means the device you are holding, and a page that forbids embedding (`X-Frame-Options`, CSP `frame-ancestors`) shows up blank. Use Screenshot or Open in new tab for those. A short hint under the frame says so.
