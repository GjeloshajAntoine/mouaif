...
- **Change resolution** — the full-screen viewer has a native **Size** selector beside the close button (Phone / Phone+ / Tablet / Laptop). Picking an entry re-captures the same URL immediately, without a model round-trip. A custom size requested by the model remains visible as the selected value.
...
- The full-screen viewer (`WebpreviewModal.jsx`) shows the capture resolution in its footer meta and exposes a **Refresh** button plus a native **Size** selector that both call `POST /api/tools/webpreview` (implemented in `src/server-handlers-tools.js`) to re-capture at a chosen size. That endpoint goes through the same `webpreview` authorization gate as the model path.
