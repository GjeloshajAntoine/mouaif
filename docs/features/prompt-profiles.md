...
- **Tool-declaration reduction** (the core of this feature): the profile controls how much of each tool is advertised to the model, not just the system-prompt text.
  - `very-small`: the first upstream request advertises only `discover_tool`; its description lists available tool names. The model calls `discover_tool({ toolName })` to get one chosen tool's full description and `parameters`. Later turns advertise `discover_tool` plus every discovered tool's full schema.
...
- **Read endpoints**:
  ...
  - `GET /api/chats/:id/tool-preview?projectDir=…` returns `{ profile, reduced, shellEnabled, count, tools: [{ name, description, hasSchema, params }] }` — the **tool-declaration state** for the chat's resolved profile. The server collects the tools exactly as `ai.streamChat` does (native `shell`, gated by `tools.shell.enabled`, plus ready MCP servers), then applies the same `reduceToolSpecs` reduction the stream will apply, so the preview is always what the model actually gets. `reduced` is `true` for `very-small`; initially this preview contains `discover_tool` with an enum of available tool names. The chat UI renders this as the creation-time preview under the S/M/L switch.
...
- Mobile UI changes (in [frontend/src/components/Chat.jsx](../../frontend/src/components/Chat.jsx)):
