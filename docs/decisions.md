...
- The Node server on port `5732` serves the built mobile UI at the root `/` (no `/web/` prefix). A legacy `/web/` route 301-redirects to `/` so old bookmarks and installed PWAs keep working (the hash fragment is preserved, so `/web/#/chat/<id>` lands on `/#/chat/<id>`).
