...
- Caps (overridable per-app, in the app SQLite store): `fileReadMaxBytes` (256 KB, bypassed by `startLine` / `endLine` slices), `fileListMaxEntries` (1000), `fileSearchMaxMatches` (200), `fileSearchMaxBytes` (2 MB), `fileWriteMaxBytes` (1 MB). The defaults are deliberately tight so a model that asks for "every file in the repo" cannot blow the budget.
...
- **Sessions are keyed by context**: the project dir when one is in scope, or the shared `'app'` context when a server is started without a project (the Settings App tab). A chat dispatches through its own context first, then `app`, so an App-tab-started server is reachable from every chat and each project still gets its own child when started from a chat.
...
- **Settings UI**: **Settings → MCP servers** moves to the *Application* group with an **App / Project** scope switcher; the Project tab shows the merged list plus the per-server authorization rows, the App tab shows app-wide servers only. The add/edit form gains a scope segmented control (add only).
- Updated code: [src/mcp.js](../src/mcp.js) (scope-aware registry, `resolveMerged`, `findServerAnyScope`, context-keyed sessions), [src/index.js](../src/index.js) (`handleMcp`), [src/web/src/components/SettingsMcp.jsx](../src/web/src/components/SettingsMcp.jsx), [src/web/src/components/SettingsHome.jsx](../src/web/src/components/SettingsHome.jsx), [src/web/src/router.js](../src/web/src/router.js). New test: [scripts/test-mcp-scope.js](../scripts/test-mcp-scope.js) (27 assertions: app add, merged listing, slug shadowing, scope-routed update/remove, app-only listing). Docs: [docs/features/mcp.md](../features/mcp.md).
