...
// and the REST/SSE handlers; the pieces live in focused sub-modules:
//
//   server-init.js    — requires + constants + provider registration
//   server-sockets.js — socket tracking, SSE client set, running-chat
//                       registry, redaction helpers
//   server-json.js    — sendJSON
//   server-settings.js— settingsForClient (client redaction)
//   server-sse.js     — handleSSE + broadcast
//   server-auth.js    — cookie/origin/access authorization helpers
//   routes.js         — handleRequest (the route table)
//   handlers-*.js     — one file per REST surface
//   server.js         — createServer (http server + WS upgrade)
//   server-seed.js    — seedModelListCache (test hook)
//   web-static.js     — /web/ static serving
//
