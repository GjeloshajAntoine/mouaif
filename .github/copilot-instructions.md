...
- **Access authentication** — optional single-user password + WebAuthn protection, enabled by CLI flags. Protection can be turned **off from inside the app** behind a one-time code and QR code (`Settings → Access & passkeys`): `POST /api/access/disable/code` mints it, `GET /api/access/disable/qr` renders it, `/#/disable-access?code=…` (reachable without a session) confirms it, and `POST /api/access/enable` re-arms with a session, the account password, or a setup code. The switch is persisted in `access_state` and takes effect in the running worker. Both directions broadcast an access on/off notification on the `login` channel. See [docs/features/access-authentication.md](../docs/features/access-authentication.md) and [docs/features/disable-access-qr.md](../docs/features/disable-access-qr.md).
