...
- **AI providers** — six ship today: OpenAI-compatible (any OpenAI-shaped endpoint, key optional so a local llama.cpp `llama-server` / LM Studio works keyless — see [docs/features/local-openai-servers.md](../docs/features/local-openai-servers.md)), Anthropic (key or OAuth), Google Gemini (key), Ollama (no key), **OpenRouter** (key or PKCE sign-in, OpenAI-shaped, OpenRouter model slugs as `id`), and GitHub Copilot (OAuth-only: GitHub device-code sign-in, per-chat Copilot token exchange, live model list — see [docs/features/github-copilot.md](../docs/features/github-copilot.md)). Adding a new provider is a localized change: a `buildRequest` + `parseEvent` in [src/ai.js](../src/ai.js), a row in [frontend/src/api.js](../frontend/src/api.js) `SETTINGS_PROVIDERS`, and (if the auth shape differs) an entry in [src/auth.js](../src/auth.js) `AI_TO_AUTH_PROVIDER`. Providers that need their own sign-in flow (PKCE / OAuth / device code) add a `src/oauth-<name>.js` module that exports `register()` and call it from [src/index.js](../src/index.js) at startup. See [docs/features/ai-client.md](../docs/features/ai-client.md) and [docs/features/openrouter.md](../docs/features/openrouter.md).
- **MCP OAuth sign-in** — remote MCP servers (Streamable HTTP or legacy SSE transport) support authorization-code PKCE (dynamic registration, public, or confidential clients with a keychain-stored secret) and the client-credentials grant, keychain-backed tokens with refresh, and RFC 7009 remote revocation on Disconnect. See [docs/features/mcp-oauth.md](../docs/features/mcp-oauth.md).
- **Dictation** — speech-to-text with a per-app model choice: the `#/dictation` page (record, transcribe, edit, copy / insert / send) and a microphone button in the chat composer that transcribes **while the user speaks** (a 3 s timeslice per request, the results joined into one growing draft; switchable on the page's Options row). Every request goes to `POST /api/ai/transcribe`, so the provider credential stays server-side. Three request families: OpenAI-shaped multipart `/audio/transcriptions`, OpenAI-shaped inline audio on `/chat/completions` (for an audio-input model with no transcription entry — on OpenRouter that is every Google Gemini row, which `/audio/transcriptions` rejects with `400 Model … does not exist`), and native Gemini `generateContent`. See [docs/features/dictation.md](../docs/features/dictation.md) and [docs/decisions.md](../docs/decisions.md) §27.
...
- **Three prompt-size profiles**:
  ...
  - `chat`: empty system prompt (a plain conversation). Like the other profiles it is only a prompt-style choice; it does not change the chat's tool selection.
...
- **Hide file content** — per-project read/search redaction with a dedicated numbered-file preview, tap-to-select lines, manual ranges, and retry-safe saving. Not a security boundary: shell, MCP, and other access are not filtered. See [docs/features/hide-file-content.md](../docs/features/hide-file-content.md).
